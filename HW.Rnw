...
cross <- function(data, f=10, k=2) {
    eclusts <- equiv.clust(data)
    BlockM <- blockmodel(data, eclusts, k=k)
    ...
    for (i in 1:f) {
        ...
        m <- btergm(nigeriaDyn2 <- edge+nodecov('member'))
        edgeprob(m)
        
        
        
        tot_pr <- c(tot_pr, pr.curve( member_or[folds[[i]]], member)$auc.integral)
        tot_roc <- c(tot_pr, roc.curve( member_or[folds[[i]]], member)$auc.integral)
...
BlockM <- blockmodel(nigeriaDyn, eclusts, k=3)
member_or <- BlockM$block.membership[BlockM$order.vec]

nigeriaDyn%v%"member" <- member_or
#nigeriaDyn %v% "member"
nigeriaDyn %v% "col" <- c("gray70", "tomato", "gold")[nigeriaDyn %v% "member"]
